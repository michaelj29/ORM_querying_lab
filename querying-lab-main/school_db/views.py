...
def problem_two(request):
 ...
 instructors = Instructor.objects.all().filter(hire_date__lt=datetime.date(2010,1,1))
 for instructor in instructors:
   if instructor.first_name == 'Jackie':
      pass
   else:
...
def problem_five(request):
 new_student = Student.objects.create(id=11, first_name = 'Kyle', last_name='Harwood', year=2022, gpa=3.0)
...
def problem_six(request):
  Student.objects.all().filter(id=11).update(gpa=3.5)
  student = Student.objects.all().get(id=11)
...
def problem_seven(request):
    ...
    student_id = 11
    Student.objects.all().filter(id=11).delete()
